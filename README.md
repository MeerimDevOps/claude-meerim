# MEERIM - Personal Branding Expert Website

A modern, professional website for a Personal Branding Expert, designed to help entrepreneurs and professionals build a strong online presence.

## Features
- **Modern Design:** Instagram-inspired aesthetic with pink-to-purple gradients.
- **Responsive:** Fully mobile-friendly layout.
- **Sections:** Home, About, Services, Portfolio, Testimonials, and Contact.
- **Call to Actions:** "Start Building Your Brand" and "Book a Consultation" buttons.
- **Performance:** Built with Tailwind CSS for fast loading and clean code.

## Deployment

This website is configured to deploy automatically via GitHub Actions.

### To enable deployment:
1. Go to your repository settings on GitHub.
2. Navigate to **Pages** (under the "Code and automation" section).
3. Under **Build and deployment > Source**, select **GitHub Actions** from the dropdown menu.

Once selected, the site will automatically deploy to:
`https://meerimdevops.github.io/claude-meerim/`

## Tech Stack
- HTML5
- Tailwind CSS
- Google Fonts (Playfair Display, Montserrat)
- Font Awesome (Icons)
- GitHub Actions (CI/CD)
